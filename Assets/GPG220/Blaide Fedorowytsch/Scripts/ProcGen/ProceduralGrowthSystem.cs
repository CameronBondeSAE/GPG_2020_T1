...
namespace GPG220.Blaide_Fedorowytsch.Scripts.ProcGen
{
	public class ProceduralGrowthSystem : MonoBehaviour
	{
		...
		// CHECK: Does this need to be called for pathfinding?
		public event Action UpdateProceduralGrowthEvent;
		...
		private void Awake()
		{
			// So you can't change the grid size while running
			gridSizeLocker = gridSize;

			gameManager       = FindObjectOfType<GameManager>();
			rtsNetworkManager = FindObjectOfType<RTSNetworkManager>();

			rtsNetworkManager.StartedHostEvent += GameStarted;
		}
		...
		public void GrowObstacleBoolGrid()
		{
			...
			foreach (Vector2Int gridPosition in openEdges)
			{
				float perlinChance = ((Mathf.PerlinNoise((gridPosition.x + seed.x) * obstacleGenerationDensity.x,
														 (gridPosition.y + seed.y) * obstacleGenerationDensity.y) -
									   0.1f) *
									  perlinWeight);
			...
			for (int x = 0; x < gridSize.x; x++)
			{
				for (int y = 0; y < gridSize.y; y++)
				{
					ObjectGrid[x, y] = Instantiate(obstaclePrefab,
												   transform.position +
												   new Vector3((x) * 0.01f * (worldSize.x / (gridSize.x * 0.01f)),
															   obstacleHeight / 2,
															   (y) * 0.01f * (worldSize.y / (gridSize.y * 0.01f))),
												   transform.rotation);
		...
		private void OnDrawGizmosSelected()
		{
			if (gizmos)
			{
				foreach (Vector2Int v in openEdges)
				{
					...
					Gizmos.DrawSphere(
									  transform.position +
									  new Vector3(v.x * (worldSize.x * 0.01f), 8, v.y * (worldSize.y * 0.01f)),
									  2f);
				}
